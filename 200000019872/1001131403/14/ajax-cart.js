	...
	var $body = $(document.body),
			params = {
				...
				success: function(line_item) {
					...
					$(".quickview-cart").addClass("drawer--is-open");
					//if ((typeof callback) === 'function') {
  ...
  ==============================================================================*/
	...
	formOverride = function () {
		...
		$("#AddToCart").on('click', function(evt) {
			...
			HaravanAPI.addItemFromForm($formContainer, itemAddedCallback, itemErrorCallback);
		});
	};
	...
	itemErrorCallback = function (XMLHttpRequest, textStatus) {
		...
		if (!!data.message) {
			if (data.status == 422) {
				$formContainer.after('<div class="errors qty-error">Bạn đã nhập quá số lượng tồn kho.</div>')
			}
		}
	};
